...
/**
 * Tests for <code>{@link SwingRadioButtonFixture}</code>.
 *
 * @author Alex Ruiz
 */
@Test
public class SwingRadioButtonFixtureTest extends SwingComponentNodeTestCase {

  private SwingRadioButtonFixture fixture;
  private FXNode node;
  private AbstractSwingButtonDriver<JRadioButton> driver;

  @SuppressWarnings("unchecked")
  void setUpFixture() {
    node = radioButton().withId("id").createNew();
    driver = createMock(AbstractSwingButtonDriver.class);
    fixture = new SwingRadioButtonFixture(robot(), node);
  ...
  public void shouldReturnNodeComponent() {
    assertThat(fixture.component()).isSameAs(componentInNode(node, JRadioButton.class));
  }
