...
/**
 * Understands a stub of <code>{@link OSIdentifier}</code>.
 *
 * @author Alex Ruiz
 */
class OSIdentifierStub extends OSIdentifier {
  ...
  private static class SystemPropertyReaderStub extends SystemPropertyReader {
    ...
    @Override String systemProperty(String propertyName) {
      if ("os.name".equals(propertyName)) return osName;
      if ("mrj.version".equals(propertyName)) return mrjVersion;
