...
public class BasicJTreeCellReader implements JTreeCellReader {
  ...
  /**
   * Creates a new </code>{@link BasicJTreeCellReader}</code> that uses a <code>{@link BasicCellRendererReader}</code>
   * to read the value from the cell renderer component in a {@code JTree}.
   */
  ...
  public BasicJTreeCellReader(CellRendererReader reader) {
    if (reader == null) throw new NullPointerException("CellRendererReader should not be null");
